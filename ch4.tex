...
In Chapter 3 we used the traditional 5 point stencil approximation to the Laplacian. At a given sub fault (position $i,j$) the 4 neighboring sub faults are used to compute the finite difference Laplacian of a given component of  slip $m_{ij}$ as
\begin{equation}
\label{eq_laplace1}
\nabla^2m_{i,j}=\frac{-4u_{i,j}+u_{i+1,j}+u_{i-1,j}+u_{i,j+1}+u_{i,j-1}}{h}\;,
\end{equation}
where $h$ is the distance between the center's of the sub-faults, If it is constant then the term can be treated as unity. A survey of the literature will reveal that seldom do authors indicate what course of action to take at the edges of the model. At the edges of the fault one can simply drop one term of Equation \ref{eq_laplace1} (or two if you are at a corner). For example at the top edge of the fault model (position $i,1$) we would write
\begin{equation}
\label{eq_laplace2}
\nabla^2m_{i,0}=\-4u_{i,1}+u_{i+1,1}+u_{i-1,1}+u{i,2}+0}\;,
\end{equation}

This is equivalent to assuming that there exists a ghost cell beyond the limits of the model and that slip not eh ghost cell is zero.
